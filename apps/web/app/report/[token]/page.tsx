...
export default function PublicReportPage({
  ...
  const metaLeads = data.leads.filter((l) => {
    ...
    return (
      isMetaSource(src) && isPaidMedium((l.utm_medium || "").toLowerCase())
    );
  });
  const googleLeads = data.leads.filter((l) => {
    const src = (l.utm_source || l.referrer_hostname || "").toLowerCase();
    return (
      isGoogleSource(src) && isPaidMedium((l.utm_medium || "").toLowerCase())
    );
  });
  ...
  const leadCountByCampaign = (() => {
    ...
    data.leads.forEach((l) => {
      ...
      data.campaign_data.forEach((c) => {
        ...
        if (
          c.campaign_id === utm ||
          c.campaign_name.toLowerCase().includes(utm.toLowerCase())
        ) {
          map[key] = (map[key] || 0) + 1;
        }
      });
    ...
    <div className="rpt text-slate-800 antialiased min-h-screen relative overflow-x-hidden bg-[#f8fafc]">
      ...
      <div className="max-w-[1440px] mx-auto p-4 sm:p-6 lg:p-8">
        ...
        {(hasAnyCampaigns || (data.metrics.total_revenue || 0) > 0) && (
          <div
            className="flex space-x-6 border-b border-slate-200 mb-8 overflow-x-auto anim-slide-up d100"
                ...
                {metaSummary.results > 0 && metaSummary.cost > 0 && (
                  <KPICard
                    ...
                    value={formatCurrency(
                      metaSummary.cost / metaSummary.results,
                    )}
              ...
              <>
                ...
                {googleSummary.results > 0 && googleSummary.cost > 0 && (
                  <KPICard
                    label="Google CPR"
                    value={formatCurrency(
                      googleSummary.cost / googleSummary.results,
                    )}
                    icon={<Target className="w-5 h-5" />}
                    iconBg="bg-emerald-50"
                    iconColor="text-emerald-600"
                  ...
                  <div className="flex flex-col justify-center gap-5">
                    {devPcts.map((d, i) => {
                      const ic =
                        ...
                        d.name.toLowerCase().includes("mobiel") ? (
                          <Smartphone className="w-3.5 h-3.5" />
                        ) : d.name.toLowerCase().includes("desktop") ? (
                    ...
                    <div>
                      ...
                      <div className="space-y-1.5">
                        {data.browsers.slice(0, 5).map((b) => {
                          const tot = data.browsers.reduce(
                            (a, x) => a + x.value,
                            0,
                          );
                          const pct =
                            tot > 0 ? Math.round((b.value / tot) * 100) : 0;
                          return (
                            <div
                              key={b.name}
                    ...
                    <div>
                      ...
                      <div className="space-y-1.5">
                        {data.operating_systems.slice(0, 5).map((os) => {
                          ...
                          const pct =
                            tot > 0 ? Math.round((os.value / tot) * 100) : 0;
                          return (
                            <div
                              key={os.name}
                              className="relative overflow-hidden rounded-lg p-2.5 border border-transparent hover:border-slate-200 group transition-colors"
