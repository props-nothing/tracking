...
export async function POST(request: NextRequest) {
  ...
  if (integration.credential_set_id) {
    ...
    if (credSet?.credentials) {
      ...
      resolvedCredentials = { ...credSet.credentials, ...Object.fromEntries(
        Object.entries(resolvedCredentials).filter(([, v]) => v && String(v).length > 0)
      )};
    }
  }
  ...
  try {
    ...
    // Delete existing data for this integration + period to prevent stale rows
