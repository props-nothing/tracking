    ...
    <div className="rounded-lg border bg-card">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-medium">AI-inzichten</h2>
            <p className="text-xs text-muted-foreground">
              {report ? `Laatst geanalyseerd: ${new Date(report.generated_at).toLocaleDateString()}` : 'Nog geen analyse'}
            </p>
          </div>
        </div>
      ...
      {!collapsed && (
        <div className="border-t px-6 py-5">
          ...
          {loading ? (
            <div className="py-8 text-center text-sm text-muted-foreground">AI-inzichten laden...</div>
          ) : generating ? (
            <div className="flex flex-col items-center gap-3 py-12">
              ...
              <div className="text-center">
                <p className="text-sm font-medium">Je data analyseren...</p>
                <p className="mt-1 text-xs text-muted-foreground">Dit duurt meestal 10-30 seconden</p>
              </div>
            </div>
          ) : report ? (
            <AIReportCard
            ...
            <div className="flex flex-col items-center gap-3 py-12 text-center">
              ...
              <div>
                <p className="text-sm font-medium">Nog geen AI-analyse</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {readOnly
                    ? 'AI-inzichten zijn nog niet gegenereerd voor dit rapport.'
                    : 'Klik op "Analyse genereren" om AI-gestuurde inzichten over je analysedata te krijgen.'}
                </p>
              </div>
            </div>
          )}
          ...
          {showHistory && (
            <div className="mt-6 border-t pt-4">
              <h3 className="mb-3 text-sm font-medium">Analysegeschiedenis</h3>
              {historyLoading ? (
                <p className="text-sm text-muted-foreground">Laden...</p>
              ) : history.length === 0 ? (
                <p className="text-sm text-muted-foreground">Geen eerdere analyses gevonden.</p>
              ) : (
                  ...
                  {history.map(h => (
                    ...
                    >
                      ...
                      <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">
                        {h.analysis?.summary || 'Geen samenvatting beschikbaar'}
                      </p>
                    </button>
                  ))}
                </div>
